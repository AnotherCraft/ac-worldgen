...
#define WORLDGEN_FUNCTIONS \
  ...
  FUNC(sampleOffset, 2, ((ANY, variable), (T, offset, Float3)), (ARG, valueAtOffset, 1), ":std::max(WGA_Value::Dimensionality::D2D, arg1)", (EXT, Sampling), "Returns value of `variable` sampled on a world position of (`worldPos() + offset`).") \
  ...
  \
  FUNC(sampleGradient2D, 2, ((T, value, Float), (T, dist, Float)), (T, gradient, Float2), ":std::max(WGA_Value::Dimensionality::D2D, arg1)", (EXT, Sampling), "Returns 2D gradient of `value`, calculated as `float2(val[x + dist] - val[x - dist], val[y + dist] - val[y - dist]) / (1 + dist * 2)`") \
  FUNC(sampleAvg2D, 2, ((NUM, value), (T, dist, Float)), (ARG, avg, 1), ":std::max(WGA_Value::Dimensionality::D2D, arg1)", (EXT, Sampling), "Samples at offsets (0,0), (dist,0), (-dist,0), (0,dist) and (0,-dist) and returns the average.`") \
  \
  SECTION("Biome functions") \
  FUNC(biomeParam_nearest, 1, ((ANY, var)), (ARG, var, 1), ":std::max(WGA_Value::Dimensionality::D2D, arg1)", (EXT, Biome), "**This function cannot be used in the WOGLAC language. There is a special syntax construct for biome params.**\n\nReturns value of variable $var of the nearest biome.") \
  FUNC(biomeParam_nearestSet, 1, ((ANY, var)), (ARG, var, 1), ":std::max(WGA_Value::Dimensionality::D2D, arg1)", (EXT, Biome), "**This function cannot be used in the WOGLAC language. There is a special syntax construct for biome params.**\n\nReturns value of variable $var of the nearest biome that has the variable set.") \
  FUNC(biomeParam_weighted, 2, ((NUM, var), (T, exp, Float)), (ARG, var, 1), ":std::max(WGA_Value::Dimensionality::D2D, arg1)", (EXT, Biome), "**This function cannot be used in the WOGLAC language. There is a special syntax construct for biome params.**\n\nReturns value of variable $var weighted across surrounding biomes. The $exp adjusts exponent for the individual weights (1 = linear interpolation).") \
  \
